...
from .. import _timing
from .. import utils
from .mota_plus import MOTA_plus_value, IDTR_value
class CLEAR(_BaseMetric):
    ...
    def __init__(self, config=None):
        super().__init__()
        main_integer_fields = ['CLR_TP', 'CLR_FN', 'CLR_FP', 'IDSW','IDTR','M=Pm', 'MT', 'PT', 'ML', 'Frag']
        extra_integer_fields = ['CLR_Frames']
        self.integer_fields = main_integer_fields + extra_integer_fields
        main_float_fields = ['MOTA','MOTA+', 'MOTP', 'MODA', 'CLR_Re', 'CLR_Pr', 'MTR', 'PTR', 'MLR', 'sMOTA']
    ...
    def eval_sequence(self, data):
        ...
        res['Detss']=data['num_tracker_dets']
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            #IDTR for MOTA by Abi----------------------------------------------

            is_idtr = (np.logical_not(np.isnan(prev_matched_tracker_ids))) & (
                np.equal(matched_tracker_ids, prev_matched_tracker_ids))
            res['IDTR'] =  (res['IDTR']+np.sum(is_idtr))
            #res['IDTR'] = -(data['num_tracker_dets']/data['num_timesteps']) + res['IDTR']
            #res['IDTR'] = - + res['IDTR']
            res['M=Pm'] = (res['M=Pm']+np.sum(is_idtr))
        ...
        #Calculating final IDTR: #Abi
        res['IDTR']=res['CLR_TP']-res['IDTR']
        #................................................
        #res['IDTR']=IDTR_value(res['IDTR'],res['CLR_TP'])
    ...
    def _compute_final_fields(res):
        ...
        #res['MOTA+'] = 1-((res['CLR_FN'] + res['CLR_FP'] + res['IDSW'] + res['IDTR']) / np.maximum(1.0, res['CLR_TP'] + res['CLR_FN'])) #Abi
        #res['MOTA+'] = (res['CLR_TP'] - res['CLR_FP'] - res['IDSW'] - (res['IDTR']) ) / np.maximum(1.0, res['CLR_TP'] + res['CLR_FN']) #Abi
        res['MOTA+']=MOTA_plus_value(res['CLR_FN'],res['CLR_FP'],res['IDSW'],res['IDTR'],res['CLR_TP']) #Abi
        res['MOTP'] = res['MOTP_sum'] / np.maximum(1.0, res['CLR_TP'])
        ...
        safe_log_idtr = np.log10(res['IDTR']) if res['IDTR'] > 0 else res['IDTR'] #Abi
        res['MOTAL'] = (res['CLR_TP'] - res['CLR_FP'] - safe_log_idsw) / np.maximum(1.0, res['CLR_TP'] + res['CLR_FN'])
        res['MOTA+L'] = (res['CLR_TP'] - res['CLR_FP'] - safe_log_idtr) / np.maximum(1.0, res['CLR_TP'] + res['CLR_FN']) #Abi
        return res